...
class Service_station_model extends CI_model
{
	...
	public function get_all_data(){
		return $this->db->select('reg1.service_head,reg1.ward_number,reg1.pincode,reg1.nearby_location,reg1.city,reg1.id,reg1.manager_name,reg1.manager_email,reg1.manager_contact,reg1.status,reg1.state,reg1.location_latitude,reg1.location_longitude,reg1.store_ref_address,reg1.working_hour_from,reg1.working_hour_to,reg2.category_name as place ,reg1.file_name,reg1.profile,reg1.amount_services,reg1.type,reg3.category_name as type')
						...
						->join('category_name as reg2','reg1.place = reg2.id','left')
						->join('category_name as reg3','reg1.type = reg3.id','left')
						->get()->result_array();
	}

	public function get_ward_part_data($id,$ward_num){
		$data =  $this->db->query('SELECT ser.*,cat.category_name as type_1 FROM `service_registration` as ser LEFT JOIN category_name as cat ON ser.type = cat.id WHERE place IN (SELECT `id` FROM `category_name` WHERE `parent_name` IN (' . implode(',', array_map('intval', $id)) . ')) AND ward_number = '.$ward_num.'');
		return $data->result_array();
	}

	public function get_particular_data($id){
		$data =  $this->db->query('SELECT ser.*,cat.category_name as type_1 FROM `service_registration` as ser LEFT JOIN category_name as cat ON ser.type = cat.id WHERE place IN (SELECT `id` FROM `category_name` WHERE `parent_name` = '.$id.')');
		return $data->result_array();
	}

	public function get_ward_perticular_data($id,$ward_num){
		
		$data =  $this->db->query('SELECT ser.*,cat.category_name as type_1 FROM (SELECT * FROM service_registration WHERE place IN (SELECT `id` FROM `category_name` WHERE `parent_name` = '.$id.') AND ward_number = '.$ward_num.') as ser LEFT JOIN category_name as cat ON ser.type = cat.id');
		return $data->result_array();
	}
	...
	public function get_particular_categories($where){
		if($where['reg1.ward_number'] == ''){
			return $this->db->query('SELECT ser.*,cat.category_name as type_1 FROM (SELECT reg1.*,`reg2`.`category_name` as `places`,`reg2`.`id` as `catid` FROM `service_registration` as `reg1`, `category_name` as `reg2` WHERE `reg1`.`place` = `reg2`.`id` AND `reg2`.`id` = '.$where['reg2.id'].') as ser LEFT JOIN category_name as cat ON ser.type = cat.id')->result_array();
		}else{
			return $this->db->query('SELECT ser.*,cat.category_name as type_1 FROM (SELECT reg1.*,`reg2`.`category_name` as `places`,`reg2`.`id` as `catid` FROM `service_registration` as `reg1`, `category_name` as `reg2` WHERE `reg1`.`place` = `reg2`.`id` AND `reg2`.`id` = '.$where['reg2.id'].' AND `reg1`.`ward_number` = '.$where['reg1.ward_number'].') as ser LEFT JOIN category_name as cat ON ser.type = cat.id')->result_array();
		}
	}
